...
public class ScheduleService {
    ...
    public ScheduleResponseDto createSchedule(ScheduleRequestDto scheduleRequestDto)
        ...
        try {
            ...
            if (localDate == null || time == null || content == null || done == null
                || repeatType == null) {
    ...
    public Schedule updateSchedule(Long id, ScheduleRequestDto requestDto) {
        try {
            ...
            if (newLocalDate == null || newTime == null || newContent == null || newDone == null
                || newRepeatType == null) {
                throw new MissingRequiredFieldException(
                    "Required field(s) are missing in updated schedule");
            }

            // 여기서 중복 일정 검사를 수행하고 이미 존재하는 경우 예외를 던짐
            List<Schedule> existingSchedules = scheduleRepository.findByLocalDate(newLocalDate);
            for (Schedule existingSchedule : existingSchedules) {
                if (existingSchedule.getTime().equals(newTime) &&
