...
public class WeatherController {
  ...
  public ResponseEntity<BaseResponse> getTemperatures(
      HttpServletRequest request) { // 3일치 온도&하늘상태 가져오기
    try {
      WeatherDto weatherDto = fetchDataAndProcess(request);
      ...
      List<Map<String, String>> temperatureData =
          weatherService.filterCurrentTimeAndSpecifiedDateData(infoList, "1500");
  ...
  public ResponseEntity<BaseResponse> getNextHoursWeather(
      HttpServletRequest request) { // 시간대별 온도&하늘상태 가져오기
    try {
      WeatherDto weatherDto = fetchDataAndProcess(request);
      ...
      List<Map<String, String>> filteredData =
          weatherService.filterNextData(infoList, startTime, 15);
